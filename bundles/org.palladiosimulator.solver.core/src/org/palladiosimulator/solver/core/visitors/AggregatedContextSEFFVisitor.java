...
/**
 * Visitor that builds up the context model including the aggregated usage context. Extends the
 * {@link SeffVisitor} and adds the calculation of execution frequencies. Uses the
 * {@link AggregatedContextExternalCallActionHandler} instead of the
 * {@link ExternalCallActionHandler}.
 * 
 * @author martens
 *
 */
public class AggregatedContextSEFFVisitor extends SeffVisitor {
    ...
    public AggregatedContextSEFFVisitor(ContextWrapper ctxWrp, double frequency, ServiceEffectSpecification seff,
        ...
        if (seff instanceof ResourceDemandingSEFF) {
            ...
            this.computedAggregatedUsage = createOrReuseExecutionContext(rdSEFF, allocationContext, this.usageScenario,
    ...
    /**
     * Extends {@link SeffVisitor#caseResourceDemandingSEFF(ResourceDemandingSEFF)}: stores the
     * described SEFF in the {@link ComputedAggregatedUsage}. Then calls
     * {@link SeffVisitor#caseResourceDemandingSEFF(ResourceDemandingSEFF)}.
     * 
     * {@inheritDoc}.
     */
    @Override
    ...
    /**
     * Overwrites the {@link SeffVisitor#caseExternalCallAction(ExternalCallAction)}. Does the same,
     * but uses an {@link AggregatedContextExternalCallActionHandler} to handle the call, which in
     * turn will instantiate an {@link AggregatedContextSEFFVisitor} for the next SEFF.
     */
    @Override
    public Object caseExternalCallAction(ExternalCallAction call) {

        AggregatedContextExternalCallActionHandler extCallAH = new AggregatedContextExternalCallActionHandler(this,
                this.usageScenario);
        ...
        if (seff instanceof ResourceDemandingSEFF) {
            ...
            ServiceExecutionContext calleeComputedAggregatedUsage = createOrReuseExecutionContext(rdSEFF,
                    allocationContext, this.usageScenario, 0.0);

            // the new usage is a communication partner, add communication info.
            AggregatedCommunication communication = createOrReuseAggregatedCommunication(this.computedAggregatedUsage,
                    calleeComputedAggregatedUsage);
            ...
            CommunicationLinkResourceSpecification link = findCommunicationLink(this.computedAggregatedUsage,
                    calleeComputedAggregatedUsage);
    ...
    private CommunicationLinkResourceSpecification findCommunicationLink(
        ...
        // AssemblyContext assemblyFrom =
        // callerComputedAggregatedUsage.getAssemblyContext_ServiceExecutionContext();
        // AssemblyContext assemblyTo =
        // calleeComputedAggregatedUsage.getAssemblyContext_ServiceExecutionContext();
        ...
        Allocation allocation = this.contextWrapper.getPcmInstance()
            .getAllocation();

        /*
         * AllocationContext to = null; for (AllocationContext allocationContext :
         * allocation.getAllocationContexts_Allocation()) {
         * 
         * AssemblyContext currentAssemblyContext =
         * allocationContext.getAssemblyContext_AllocationContext(); // look inside assembled
         * component to see whether the search assembly context assemblyTo in contained if
         * (isOrContainsAssemblyContext(currentAssemblyContext, assemblyTo)){ to =
         * allocationContext; } if (to != null) break; }
         */
        ...
        if (to != null) {
            ...
            List<LinkingResource> linkingResourceList = this.contextWrapper.getPcmInstance()
                .getResourceEnvironment()
                .getLinkingResources__ResourceEnvironment();
            for (LinkingResource linkingResource : linkingResourceList) {
                if (linkingResource.getConnectedResourceContainers_LinkingResource()
                    .contains(fromResourceContainer)
                        && linkingResource.getConnectedResourceContainers_LinkingResource()
                            .contains(toResourceContainer)) {
                    return linkingResource.getCommunicationLinkResourceSpecifications_LinkingResource();
                }
            }
            throw new RuntimeException("The communication between allocation contexts " + to.getEntityName() + " and "
                    + from.getEntityName()
                    + " is remote, but there is no linking resource between them. Add a linking resource.");

        }
        throw new RuntimeException(this.getClass()
            .getName()
                + ": Could not determine allocation of "
                + calleeComputedAggregatedUsage.getDescribedSEFF_ServiceExecutionContext()
                    .getDescribedService__SEFF()
                    .getEntityName()
                + " to determine communication partners.");
    ...
    /**
     * Extends {@link SeffVisitor#caseInternalAction(InternalAction)}. Adds the calculation of a
     * mean resource demand weighted by frequency of execution. Adds this information to the
     * {@link AggregatedResourceDemand}. Then calls
     * {@link SeffVisitor#caseInternalAction(InternalAction)}.
     * 
     * {@inheritDoc}
     */
    @Override
    public Object caseInternalAction(InternalAction action) {
        ...
        for (ParametricResourceDemand demand : action.getResourceDemand_Action()) {
            String specification = demand.getSpecification_ParametericResourceDemand()
                .getSpecification();
    ...
    private void addToAggregatedUsage(ResourceType resourceType, double demand) {
        ...
        List<AggregatedResourceDemand> existingResourceDemands = this.computedAggregatedUsage
        ...
        for (AggregatedResourceDemand aggregatedResourceDemand : existingResourceDemands) {
            if (aggregatedResourceDemand.getResourceType_AggregatedResourceDemand()
                .equals(resourceType)) {
        ...
        if (resourceDemandForType == null) {
            ...
            this.computedAggregatedUsage.getAggregatedResourceDemands_ServiceExecutionContext()
                .add(resourceDemandForType);
        }
    ...
    /**
     * Gets the frequency to execute this {@link ResourceDemandingBehaviour} by checking the
     * container. If it is a {@link BranchTransition} or {@link AbstractLoopAction}, update the
     * frequency before calling {@link SeffVisitor#caseExternalCallAction(ExternalCallAction)}. Then
     * resets the frequency for the next action or trasition. Then calls
     * {@link SeffVisitor#caseResourceDemandingBehaviour(ResourceDemandingBehaviour)}.
     * 
     * {@inheritDoc}.
     */
    @Override
    public Object caseResourceDemandingBehaviour(ResourceDemandingBehaviour behaviour) {
        ...
        if (container instanceof AbstractBranchTransition) {
            ...
            double branchProb = -1;
            List<BranchProbability> branchProbabilities = this.contextWrapper.getCompUsgCtx()
            ...
            if (branchProb == -1) {
                throw new RuntimeException("Internal error: Found no branch transition probability for branch "
                        + branchTransition.getEntityName() + " " + branchTransition.getId());
            }
            ...
            List<LoopIteration> loopIterations = this.contextWrapper.getCompUsgCtx()
            ...
            for (LoopIteration loopIteration : loopIterations) {
                if (loop.equals(loopIteration.getLoopaction_LoopIteration())) {
                    avgLoopIterationNumber = ExpressionHelper
                        .meanValue(ExpressionHelper.getSolvedExpression(loopIteration.getSpecification_LoopIteration()
                            .getSpecification(), this.contextWrapper));
                }
            }
            if (avgLoopIterationNumber == -1) {
                throw new RuntimeException("Internal error: Found no branch transition probability for loop "
                        + loop.getEntityName() + " " + loop.getId());
            }
    ...
    /**
     * Get the current frequency of this object. This is <i>not</i> equal to the frequency of this
     * {@link ServiceEffectSpecification}, because the current frequency reflects the frequency of
     * the currently handled {@link AbstractAction}.
     */
    public double getCurrentFrequency() {
        return this.currentFrequency;
    }

    private ServiceExecutionContext createOrReuseExecutionContext(ResourceDemandingSEFF rdSEFF,
            AllocationContext allocationContext, UsageScenario usageScenario, double frequency) {

        ServiceExecutionContext computedAggregatedUsage = null;

        List<ServiceExecutionContext> aggrUsageContext = this.contextWrapper.getPcmInstance()
            .getComputedAggregatedUsage()
            .getServiceExecutionContexts_ComputedAggregatedUsage();
        ...
        if (computedAggregatedUsage == null) {
            ...
            this.contextWrapper.getPcmInstance()
                .getComputedAggregatedUsage()
                .getServiceExecutionContexts_ComputedAggregatedUsage()
                .add(computedAggregatedUsage);
        }

        return computedAggregatedUsage;
    }

    private AggregatedCommunication createOrReuseAggregatedCommunication(ServiceExecutionContext fromService,
            ServiceExecutionContext toService) {

        AggregatedCommunication communication = null;

        List<AggregatedCommunication> communications = fromService
