...
public class CollectionIteratorActionHandler extends AbstractLoopActionHandler {
    ...
    /**
     * Fetches the expression describing the number of loop iteration from the usage context. It is
     * the NUMBER_OF_ELEMENTS of the parameter in the SEFF, whose name is passed here as input
     * parameter.
     * 
    ...
    private String getIterationExpression(String soughtParameterName) {
        EList<VariableUsage> vuList = visitor.getContextWrapper()
            .getCompUsgCtx()
        ...
        for (VariableUsage vu : vuList) { // iterate over parameters
            ...
            if (currentParameterName.equals(soughtParameterName)) {
                ...
                for (VariableCharacterisation vc : varChars) { // iterate over a parameter's
                                                               // characterisations
                ...
                logger.error("Variable Characterisation NUMBER_OF_ELEMENTS missing " + "in Usage Context for parameter "
                        + soughtParameterName + ")! " + "It is needed for a CollectionIteratorAction.");
                return null;
            }
        }
        logger.error("Variable " + soughtParameterName + " missing " + "in Usage Context!");
        return null;
    }
